...
public interface GameMapper {
    ...
    @Insert("""
            INSERT INTO torpedo_launches (room_id, torpedo_model_id, owner_model_id, kommandant_uuid)
    ...
    @Select("""
            SELECT hr.reporter_uuid, COALESCE(cs.tonnage, 0) AS tonnage
            FROM hit_reports hr
            JOIN cargo_ship_states cs ON cs.room_id = hr.room_id AND cs.model_id = hr.target_model_id
            WHERE hr.room_id = #{roomId} AND hr.target_model_id = #{cargoModelId}
            """)
    List<Map<String, Object>> selectCargoHitSettlementSource(@Param("roomId") String roomId, @Param("cargoModelId") String cargoModelId);

    @Insert("""
            INSERT INTO settlement_records (room_id, kommandant_uuid, cargo_ships_sunk, total_tonnage)
            VALUES (#{roomId}, #{uuid}, 1, #{tonnage})
            ON DUPLICATE KEY UPDATE
                cargo_ships_sunk = cargo_ships_sunk + 1,
                total_tonnage = total_tonnage + VALUES(total_tonnage)
            """)
    int incrementSettlement(@Param("roomId") String roomId, @Param("uuid") String uuid, @Param("tonnage") int tonnage);
    ...
    @Delete("""
            ...
            WHERE room_id = #{roomId} AND last_update_at < (CURRENT_TIMESTAMP(3) - INTERVAL 60 SECOND)
            """)
