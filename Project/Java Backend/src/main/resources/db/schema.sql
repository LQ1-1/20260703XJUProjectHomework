CREATE DATABASE IF NOT EXISTS uboot_game
...
CREATE TABLE IF NOT EXISTS settlement_records (
  ...
  u_boats_sunk INT NOT NULL DEFAULT 0,
...
DROP FUNCTION IF EXISTS acquire_room_write_lock;

CREATE FUNCTION acquire_room_write_lock(p_room_id VARCHAR(64), p_timeout_seconds INT)
RETURNS INT
NOT DETERMINISTIC
READS SQL DATA
RETURN GET_LOCK(CONCAT('uboat_game:room:', p_room_id), p_timeout_seconds);

DROP FUNCTION IF EXISTS release_room_write_lock;

CREATE FUNCTION release_room_write_lock(p_room_id VARCHAR(64))
RETURNS INT
NOT DETERMINISTIC
READS SQL DATA
RETURN RELEASE_LOCK(CONCAT('uboat_game:room:', p_room_id));
